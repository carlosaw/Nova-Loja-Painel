...
class Categories extends Model {
	...
	public function  getAll() {
		...
		if($sql->rowCount() > 0) {
			...
			// Loop das categorias e subs
			foreach($data as $item) {
				$item['subs'] = array();
				$array[$item['id']] = $item;
			}

			while( $this->stillNeed($array) ) {//Enquanto houver subs
	...
	private function organizeCategory(&$array) {// & associação externa
	...
	private function stillNeed($array) {// Ainda precisa
